...
import { config, isOpenAIConfigured, isGitHubConfigured, getConfigStatus } from "./config";
...
async function searchGitHubForAHKScripts(query: string, page: number = 1, perPage: number = 30): Promise<{ results: GitHubSearchResult[], totalCount: number }> {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403) {
      throw new Error('GitHub API rate limit exceeded - add a GITHUB_TOKEN for higher limits');
    }
